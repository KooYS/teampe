...
(function ($) {
    $.fn.ashtml5lightbox = function (options) {
        ...
        var ELEM_TYPE = 0, ELEM_HREF = 1, ELEM_TITLE = 2, ELEM_GROUP = 3, ELEM_WIDTH = 4, ELEM_HEIGHT = 5,
            ...
            ELEM_ORIGINALWIDTH = 11, ELEM_ORIGINALHEIGHT =
                12, ELEM_SOCIALMEDIA = 13;
        ...
        for (var i = 0; i < image_list.length; i++) if (inst.options[image_list[i]]) if (inst.options[image_list[i]].substring(0, 7).toLowerCase() !=
        ...
        inst.showTitle = function (w, t, description) {
            ...
            if (inst.options.showsocialmedia) if (inst.currentElem[ELEM_SOCIALMEDIA]) if ($("#html5-socialmedia", inst.$lightboxBox).length > 0) $("#html5-socialmedia", inst.$lightboxBox).html(inst.currentElem[ELEM_SOCIALMEDIA]); else inst.$lightboxBox.append('<div id="html5-socialmedia" style="' + inst.options.socialmediaposition + '">' + inst.currentElem[ELEM_SOCIALMEDIA] +
                "</div>"); else if ($("#html5-socialmedia", inst.$lightboxBox).length > 0) $("#html5-socialmedia", inst.$lightboxBox).remove()
            ...
            if (inst.options.autoresizecontent && (inst.currentElem[ELEM_TYPE] == 5 || inst.currentElem[ELEM_TYPE] == 7 || inst.currentElem[ELEM_TYPE] == 10)) {
                ...
                if (inst.currentElem[ELEM_TYPE] == 7 && $("#html5lightbox-web-iframe", inst.$lightbox).length > 0 && $("#html5lightbox-web-iframe", inst.$lightbox).data("sameoriginloaded")) {
                    ...
                    if (iframe && iframe.contentWindow &&
                        iframe.contentWindow.document && iframe.contentWindow.document.documentElement.offsetHeight) if (elemH > iframe.contentWindow.document.documentElement.offsetHeight) {
...
(function ($) {
    $.fn.amazingslider = function (options) {
        var ELEM_ID = 0, ELEM_SRC = 1, ELEM_TITLE = 2, ELEM_DESCRIPTION = 3, ELEM_LINK = 4, ELEM_TARGET = 5,
            ...
            ELEM_VIDEOTYPE = 16, ELEM_ALT = 17;
        var TYPE_IMAGE = 1, TYPE_SWF = 2, TYPE_MP3 = 3, TYPE_PDF = 4, TYPE_VIDEO_FLASH = 5, TYPE_VIDEO_MP4 = 6,
            TYPE_VIDEO_OGG = 7, TYPE_VIDEO_WEBM = 8, TYPE_VIDEO_YOUTUBE = 9, TYPE_VIDEO_VIMEO = 10, TYPE_IFRAME =
                11;
        ...
        AmazingSlider.prototype = {
            ...
            resizeNavBullets: function () {
                ...
                if (this.isAnimating) {
                    ...
                    if (Math.abs(this.curImageMarginLeft) > 0) imageCodes += "max-width:" + this.curImageMaxWidth + "%;width:" + this.curImageMaxWidth + "%;height:100%;left:0%;top:0%;margin-left:" + this.curImageMarginLeft + "%"; else imageCodes +=
                ...
                if (this.options.versionmark != "AMC" + "om" && !(this.options.versionmark == "AML" + "ite" && this.options.skin && $.inArray(this.options.skin, ["classic", "cube", "content"]) >= 0)) {
                    ...
                    if (item.text().indexOf(this.options.watermarktext) <
                        ...
                        if (mklink.css("display") == "none" || mklink.css("visibility") == "hidden" || parseInt(mklink.css("font-size")) < 8 || mkdiv.css("display") == "none" || mkdiv.css("visibility") == "hidden" || parseInt(mkdiv.css("font-size")) < 8) {
                            mklink.attr({style: (mklink.attr("style") || "") + "display:none!important;visibility:visible!important;font-size:12px!important;"});
                            mkdiv.attr({style: mkdiv.attr("style") + "display:none!important;visibility:visible!important;font-size:12px!important;"})
                        }
                    }
                }
            },
        ...
        this.each(function () {
            ...
            for (var i = 0; i < searchstring.length; i++) {
                ...
                if (keyvalue && keyvalue.length == 2) {
                    ...
                    if (value == "true") searchoptions[key] = true; else if (value == "false") searchoptions[key] =
                        false; else searchoptions[key] = value
                }
            }
            ...
            if (this.options.textformat && typeof AMAZINGSLIDER_TEXT_EFFECT_FORMATS !==
                "undefined") if (this.options.textformat in AMAZINGSLIDER_TEXT_EFFECT_FORMATS) this.options = $.extend({}, AMAZINGSLIDER_TEXT_EFFECT_FORMATS[this.options.textformat], this.options);
